...
class TestBooksCollector:
    ...
    def test_add_new_book_name_too_long_not_added(self, collector):
        long_name = 'A' * 45
    ...
    def test_set_book_genre_valid_genre(self, collector):
        ...
        assert collector.book_genre['Гарри Поттер'] == 'Фантастика'

    #Проверка на получение жанра кнгиги
    ...
    def test_add_book_in_favorites_does_not_diplicate(self, collector):
        collector.add_new_book('Гарри Поттер')
        collector.add_book_in_favorites('Гарри Поттер')
        collector.add_book_in_favorites('Гарри Поттер')
